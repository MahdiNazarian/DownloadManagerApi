...
class DbUserDownloadMapping(Base):
    __tablename__ = "DbUserDownloadMapping"
    ...
    DownloadFileId = Column(BigInteger, ForeignKey("DbDownloadFile.Id"), nullable=False)
    DownloadStartDate = Column(DATETIME2, default=datetime.datetime.now(), nullable=False)
    DownloadEndDate = Column(DATETIME2, default=datetime.datetime.now(), nullable=False)
    User = relationship("DbUser")
    DownloadFile = relationship("DbDownloadFile")

    @staticmethod
    def from_json_update(json_dct):
        return DbUserDownloadMapping(
            ...
            DownloadFileId=json_dct["DownloadFileId"],
            DownloadStartDate=json_dct["DownloadStartDate"],
            DownloadEndDate=json_dct["DownloadEndDate"]
        )
